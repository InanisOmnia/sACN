...
export namespace Sender {
	export interface Props {
		...
		/**
		 * Set this option to attempt to give individual addresses their own priorities
		 */
		perAddressPriority?: Record<number, number>;
	}
...
export class Sender extends EventEmitter {
	...
	public readonly perAddressPriority: Sender.Props["perAddressPriority"];
	...
	public send(packet: Omit<Options, "sequence" | "universe">): Promise<void> {
        ...
        const papPacket: any = {...this.defaultPacketOptions, ...packet}
        delete papPacket.payload;
				const pack = new Packet({
					...papPacket,
					payload: this.perAddressPriority,
        ...
        // console.log("PAP", pack)
				...
				this.socket.send(pack.buffer as unknown as string, this.port, this.#destinationIp, (err) =>
