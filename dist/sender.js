...
class Sender extends events_1.EventEmitter {
    constructor({ universe, port = 5568, reuseAddr = false, minRefreshRate = 0, defaultPacketOptions, iface, useUnicastDestination, perAddressPriority }) {
        super();
        /**
        ...
        this.perAddressPriority = perAddressPriority;
    ...
    send(packet) {
        ...
        return new Promise((resolve, reject) => {
            if (this.perAddressPriority) {
                const papPacket = Object.assign(Object.assign({}, this.defaultPacketOptions), packet);
                delete papPacket.payload;
                const pack = new packet_1.Packet(Object.assign(Object.assign({}, papPacket), { payload: this.perAddressPriority, universe: this.universe, sequence: this.sequence }));
                pack.startCode = 0xdd;
                // console.log("PAP", pack)
                this.sequence = (this.sequence + 1) % 256;
                this.socket.send(pack.buffer, this.port, __classPrivateFieldGet(this, _Sender_destinationIp, "f"), (err) => err ? reject(err) : resolve());
            }
            const pack = new packet_1.Packet(Object.assign(Object.assign({}, finalPacket), { universe: this.universe, sequence: this.sequence }));
            this.sequence = (this.sequence + 1) % 256;
            // console.log("DMX", pack)
            this.socket.send(pack.buffer, this.port, __classPrivateFieldGet(this, _Sender_destinationIp, "f"), (err) => err ? reject(err) : resolve());
        });
    }
    reSend() {
